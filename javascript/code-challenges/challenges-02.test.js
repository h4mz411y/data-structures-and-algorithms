...
CHALLENGE 1 - Review
ch of those numbers raised to the 3rd power (hint: look up Math.pow()). Use forEach to solve this problem.
Write a function named raisedToTheThird that takes in an array of numbers and returns a new array of ea
...
const addOne = (arr) => {
  let newArray = arr.map ((elmt) => elmt+1)
  return newArray;
};
...
const addQuestion = (arr) => {
  let newArray = arr.map ((elmt) =>  elmt + "?")
  return newArray
};
...
const forLoopTwoToThe = (arr) => {
  ...
  let newArray = arr.map((elmt)=> Math.pow(2, elmt))
  return newArray;
